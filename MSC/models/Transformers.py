...
class SCCLBert(nn.Module):
    def __init__(self, bert_model, tokenizer, cluster_centers=None, alpha=1.0,num=0):
        ...
        self.cluster_head = nn.Sequential(
            nn.Linear(self.emb_size, self.emb_size),
            nn.ReLU(inplace=True),
            nn.Linear(self.emb_size, self.clusternum),
            nn.Softmax(dim=1))
        # Clustering head
    ...
    def cluster_logits(self, embd1, embd2=None):
