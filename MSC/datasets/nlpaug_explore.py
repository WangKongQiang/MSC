...
def contextual_augment(data_source, data_target, textcol="text", aug_p=0.2, device1="cuda", device2="cuda"):
    ...
    augmenter1 = naw.ContextualWordEmbsAug(
        model_path='models--FacebookAI--roberta-base', action="substitute", aug_min=1, aug_p=aug_p, device=device1)
    
    augmenter2 = naw.ContextualWordEmbsAug(
        model_path='models--google-bert--bert-base-uncased', action="substitute", aug_min=1, aug_p=aug_p, device=device2)
        
    train_data = pd.read_csv(data_source, encoding='ISO-8859-1')
...
def augment_files(datadir="./", targetdir="./", dataset="wiki1m_unique", aug_p=0.1, augtype="trans_subst"):
    ...
    if augtype == "word_deletion":
        ...
        # augseq = randomchar_augment(DataSource, DataTarget, textcol="text", aug_p=aug_p, augstage="init")
    else:
...
# augstage init
    ...
    #     augment_files(datadir=datadir, targetdir=targetdir, dataset=dataset, aug_p=0.1, augtype="charswap")
    #     augment_files(datadir=datadir, targetdir=targetdir, dataset=dataset, aug_p=0.2, augtype="charswap")

# augstage post

    # datadir = "textdatasets/augmentation"
    # targetdir = "textdatasets/augmentation"
    # datasets = ["agnews_charswap_10", "searchsnippets_charswap_10", "stackoverflow_charswap_10", "biomedical_charswap_10",
    #             "googlenews_TS_charswap_10", "googlenews_T_charswap_10", "googlenews_S_charswap_10", "tweet_charswap_10"]
    # for dataset in datasets:
    #     augment_files(datadir=datadir, targetdir=targetdir, dataset=dataset, aug_p=0.1, augtype="charswap")
    #
    # datasets = ["agnews_charswap_20", "searchsnippets_charswap_20", "stackoverflow_charswap_20", "biomedical_charswap_20", "googlenews_TS_charswap_20", "googlenews_T_charswap_20", "googlenews_S_charswap_20", "tweet_charswap_20"]
    # for dataset in datasets:
    #     augment_files(datadir=datadir, targetdir=targetdir, dataset=dataset, aug_p=0.2, augtype="charswap")
...
# Perform the charswap operation

    datadir = "textdatasets/augmentation"
    targetdir = "textdatasets/augmentation"

    datasets = ["agnews_trans_subst_10", "searchsnippets_trans_subst_10", "stackoverflow_trans_subst_10", "biomedical_trans_subst_10", "googlenews_TS_trans_subst_10", "googlenews_T_trans_subst_10", "googlenews_S_trans_subst_10","tweet_trans_subst_10"]
    ...
    datasets = ["agnews_trans_subst_20", "searchsnippets_trans_subst_20", "stackoverflow_trans_subst_20", "biomedical_trans_subst_20", "googlenews_TS_trans_subst_20", "googlenews_T_trans_subst_20", "googlenews_S_trans_subst_20", "tweet_trans_subst_20"]
